...
from gplearn.functions import build_context_from_index, set_function_context
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def set_context_index(self, index):
        """
        Provide a pandas MultiIndex (order-aligned with X rows) to enable TS/CS operators.
        Must be called before fit(), and again before each predict() on a new dataset.
        """
        ctx = build_context_from_index(index)
        set_function_context(ctx)
        self._has_ts_cs_context = True
